...
local function _CreatePed(hash, coords, scenario)
    ...
    SetEntityCanBeDamaged(ped, false) -- Burada `npc` değil `ped` kullanılmalı
...
CreateThread(function()
    QBCore.Functions.TriggerCallback('disi-emlaksc:server:getConfig', function(Config)
        if Config and Config.Traders then
                        ...
                        options = {
                            {
                                ...
                                action = function(entity)
                                    ...
                                    local lisenseLabel = v.license.label -- Burada doğru yazım `label`
                                    
                                    if license then
                                        QBCore.Functions.TriggerCallback('disi-emlaksc:server:getLicense', function(licenseTable)
                                            if licenseTable[license] then
                                                ...
                                                TriggerEvent('disi-emlaksc:client:OpenMenu')
                                            else
...
-- Events
RegisterNetEvent('disi-emlaksc:client:OpenMenu', function()
    ...
    if buyItems then
        openShop[#openShop +1] = {
            ...
            params = {
                event = 'disi-emlaksc:client:OpenSellMenu',
                args = {
    ...
    if sellItems then
        openShop[#openShop +1] = {
            ...
            params = {
                event = 'disi_emlaksc:client:OpenShop',
            }
        }
    end
    ...
    openShop[#openShop +1] = {
        ...
        params = {
            event = 'qb-menu:closeMenu' -- Bu bir fonksiyon çağrısı olmalı
        }
    }
...
RegisterNetEvent('disi-emlaksc:client:OpenSellMenu', function(data)
    QBCore.Functions.TriggerCallback('disi-emlaksc:server:getInv', function(inventory)
        ...
        for k,v in pairs(inventory) do
			for i = 1, #data.items do
				...
				sellMenu[#sellMenu +1] = {
				    ...
				    params = {
				        event = 'disi-emlaksc:client:SellItems',
							args = {
        ...
        sellMenu[#sellMenu+1] = {
			...
			params = {
				event = 'disi-emlaksc:client:OpenMenu'
			}
		}
...
RegisterNetEvent('disi-emlaksc:client:SellItems', function(item)
	...
	if sellingItem then
		...
		if tonumber(sellingItem.amount) > 0 then
			TriggerServerEvent('disi-emlaksc:server:SellItems', item.name, sellingItem.amount, item.price)
		else
...
RegisterNetEvent('disi_emlaksc:client:OpenShop', function()
    local ShopItems = {}
